       ...
       const nfts = await Promise.all(
         potentialNFTsByOwnerAddress.map(async ({ mintAddresses }) => {
           ...
           for (let i = 0; i < nonNullInfos.length; i ++) {
             ...
             if (!creators || !creators.length || creators.length === 0) {
               continue;
             }

             if (filters.find(filter => {
                 if (filter.candyMachineCreator) {
                   return creators[0].address === filter.candyMachineCreator && creators[0].verified;
                 }
                 else {
                   return metadata.updateAuthority === filter.updateAuthority &&  metadata?.data?.name.indexOf(filter.collectionName) >= 0;
                 }
               })) {
               metadataList.push({
                 ...metadata,
...
export async function getNftInfo(mintAddress:any) {
